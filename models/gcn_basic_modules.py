...
from torch.nn import Sequential, Dropout, Linear, ReLU, BatchNorm1d, Parameter, LayerNorm


def MLP(channels, batch_norm=True):
    if batch_norm: # BatchNorm1d: normalize along the vertex feature dimension
        # return Sequential(*[Sequential(Linear(channels[i - 1], channels[i]), ReLU(), BatchNorm1d(channels[i], momentum=0.1))
        #                     for i in range(1, len(channels))])
        return Sequential(*[Sequential(Linear(channels[i - 1], channels[i]), ReLU(), LayerNorm(channels[i]))
                        for i in range(1, len(channels))])
        # return Sequential(*[Sequential(Linear(channels[i - 1], channels[i]), ReLU(), BatchNorm1d(channels[i], momentum=0.1)) # mTODO3
        #                 for i in range(1, len(channels))])
    else:
...
class EdgeConv(MessagePassing):
    def __init__(self, in_channels, out_channels, nn, aggr='max', **kwargs): # mTODO5: aggr='mean', 'min'
